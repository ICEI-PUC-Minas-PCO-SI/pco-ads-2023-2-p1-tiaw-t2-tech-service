...
function receberdados() {
  ...
  if (recuperarUsuario[0].Vaga[0] === 0) {
    recuperarUsuario[0].Vaga[0].push(novaVaga);
  } else {
    recuperarUsuario[0].Vaga.push(novaVaga);
  }
...
function criarVaga() {
  ...
  if (vagaaDados && vagaaDados[0].Vaga) {
    console.log(vagaaDados.Vaga);
    for (let i = 0; i < vagaaDados[0].Vaga.length; i++) {
      const vagaAtual = vagaaDados[0].Vaga[i];
      const vagaIMG = vagaaDados[0].imagem;
...
function removerVaga(identificador) {
  ...
  if (vagaaDados && vagaaDados[0].Vaga) {
    ...
    vagaaDados[0].Vaga.splice(indice, 1);
